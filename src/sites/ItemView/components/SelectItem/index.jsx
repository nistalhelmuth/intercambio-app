import React from 'react';
import { PropTypes } from 'prop-types';
import { connect } from 'react-redux';
import * as belonginsActions from '../../../../actions/belongings';
import * as selectors from '../../../../reducers';

import Stars from '../../../SharedComponents/Stars';

import './styles.css';

const Item = ({
  name,
  usedState,
  selectBelongin,
  selected,
}) => (
  <button type="button" className="button-item" onClick={selectBelongin}>
    {selected && <img src="/tools/checkmark.png" alt="check" className="check" />}
    <div className="button-content">
      <img className="picture" src="data:image/png;base64," alt="icon"/>
      <p>
        {name}
      </p>
      <Stars num={usedState} />
    </div>
  </button>
);

Item.propTypes = {
  name: PropTypes.string.isRequired,
  usedState: PropTypes.number.isRequired,
  selected: PropTypes.bool.isRequired,
  selectBelongin: PropTypes.func.isRequired,
};

export default connect(
  (state, { id }) => ({
    ...selectors.getBelonging(state, id),
  }),
  (dispatch, { id }) => ({
    selectBelongin() {
      dispatch(belonginsActions.belonginSelected(id));
    },
  }),
)(Item);
